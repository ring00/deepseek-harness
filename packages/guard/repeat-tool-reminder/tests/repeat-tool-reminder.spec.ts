...
describe('chain semantics', () => {
  ...
  it('keys chains per agent: one agent repeating never trips another', async () => {
    ...
    const agentA = (await ctx.agents.create({ sessionId: SessionId('a'), agentOptions: { provider: 'mock-a', model: 'model-a' } })).agent
    const agentB = (await ctx.agents.create({ sessionId: SessionId('b'), agentOptions: { provider: 'mock-b', model: 'model-b' } })).agent
  ...
  it('drops an agent chain on disposal', async () => {
    ...
    const fiber = await ctx.plugin(Object.assign(async (inner: Context) => {
      first = (await inner.agents.create({ sessionId: SessionId('reused'), agentOptions: { provider: 'mock', model: 'mock' } })).agent
    }, { inject: ['agents'] }))
    ...
    const second = (await ctx.agents.create({ sessionId: SessionId('reused'), agentOptions: { provider: 'mock', model: 'mock' } })).agent
