...
async function setup(script: Script, parentOptions: Partial<AgentOptions> = {}) {
  ...
  const parent = (await ctx.agents.create({ sessionId: SessionId('parent'), agentOptions: { provider: 'mock', model: 'mock', ...parentOptions } })).agent
...
describe('startInProcessRun', () => {
  ...
  it('uses explicit child model selectors when the parent has none and preserves its cwd', async () => {
    ...
    const parent = (await ctx.agents.create({ sessionId: SessionId('bare-parent'), agentOptions: {}, meta: { cwd: '/workspace' } })).agent
  ...
  it('stamps only the resolved depth when neither parent nor request declares a model route', async () => {
    ...
    const parent = (await ctx.agents.create({ sessionId: SessionId('routeless-parent'), agentOptions: {} })).agent
    const run = await startInProcessRun(request(parent), {})
    const child = ctx.agents.get(run.id)!
