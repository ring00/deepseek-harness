...
describe('hooks-codex bridge', () => {
  ...
  it('turn cancellation aborts and reaps a running UserPromptSubmit hook before idle', async () => {
    ...
    const ctx = await harness(dir, adapter)
    const agent = (await ctx.agents.create({ sessionId: SessionId('cancel-prompt-hook'), agentOptions: { provider: 'mock', model: 'mock' } })).agent
  ...
  it('an invalid regex matcher is reported and registers no hooks', async () => {
    ...
    const agent = (await ctx.agents.create({ sessionId: SessionId('invalid-codex-matcher'), agentOptions: { provider: 'mock', model: 'mock' } })).agent
    agent.followup(createUserMessage({ content: [{ type: 'text', text: 'go' }], source: { kind: 'user' } }))
    await waitForIdle(ctx, agent)
    expect(adapter.requests).toHaveLength(1)
