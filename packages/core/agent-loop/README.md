...
Declarative config uses the same asynchronous registry factory as programmatic creation, including every `ctx.agents.registerSetup()` contribution. `agents[].id` is a stable label and normally mints `${label}-session-<uuid>`; an exact `sessionId` restores materialized history when persistence contains it and otherwise creates it. `resumeSessionId` requires existing history and is mutually exclusive with `sessionId`.
...
- **Config agents have no per-row persona or setup hook** — they use the deployment persona and receive deployment-wide `ctx.agents.registerSetup()` contributions.
