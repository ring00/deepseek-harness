...
describe('request-error action edges', () => {
  it('ignores a retry action returned after the turn was aborted', async () => {
    ...
    const ctx = await harness(adapter)
    const agent = (await ctx.agents.create({ sessionId: SessionId('retry-after-cancel'), agentOptions: { provider: 'mock', model: 'mock' } })).agent
  ...
  it('completed recovery does not retry when cancellation raced the waterfall', async () => {
    ...
    const ctx = await harness(adapter)
    const agent = (await ctx.agents.create({ sessionId: SessionId('retry-raced'), agentOptions: { provider: 'mock', model: 'mock' } })).agent
...
describe('stream failure edges', () => {
  it('rethrows a mid-stream throw that carries no adapter failure facts', async () => {
    ...
    const ctx = await harness(adapter)
    const agent = (await ctx.agents.create({ sessionId: SessionId('stream-no-facts'), agentOptions: { provider: 'mock', model: 'mock' } })).agent
    let recoveries = 0
...
describe('post-turn continuation edges', () => {
  it('whenIdle resolves for a waiter whose awaited run fails', async () => {
    ...
    const ctx = await harness(adapter)
    const agent = (await ctx.agents.create({ sessionId: SessionId('whenidle-reject'), agentOptions: { provider: 'mock', model: 'mock' } })).agent
    let rejected = false
    ctx.on('internal/dispatch', (_mode, name, args) => {
      if (name !== 'session/event') return
      const event = args[1] as SessionEvent
...
describe('persistent step-close rejection', () => {
  it('still publishes the terminal status when both step-close attempts are vetoed', async () => {
    ...
    const ctx = await harness(adapter)
    const agent = (await ctx.agents.create({ sessionId: SessionId('stepend-double-veto'), agentOptions: { provider: 'mock', model: 'mock' } })).agent
...
describe('tool result meta persistence', () => {
  it('records a presentationMeta payload on the tool/result event', async () => {
    ...
    const ctx = await harness(adapter)
    const agent = (await ctx.agents.create({ sessionId: SessionId('tool-meta'), agentOptions: { provider: 'mock', model: 'mock' } })).agent
...
describe('turn close failure containment', () => {
  it('a rejected turn/end append is contained: warn + agent/error, no retry', async () => {
    const adapter = new MockAdapter([textResponse('ok')])
    const ctx = await harness(adapter)
    const agent = (await ctx.agents.create({ sessionId: SessionId('turnend-veto'), agentOptions: { provider: 'mock', model: 'mock' } })).agent
    let vetoed = false
    ctx.on('internal/dispatch', (_mode, name, args) => {
      if (name !== 'session/event') return
      const event = args[1] as SessionEvent
...
describe('recovery without a retry action', () => {
  it('a completed recovery that returns no action leaves the failed turn terminal', async () => {
    ...
    const ctx = await harness(adapter)
    const agent = (await ctx.agents.create({ sessionId: SessionId('recovery-no-retry'), agentOptions: { provider: 'mock', model: 'mock' } })).agent
    let recoveries = 0
    ctx.on('agent/request-error', async () => { recoveries += 1 })
...
describe('unrenderable failure settlement', () => {
  it('drops the rendered message when the error chain cannot be rendered', async () => {
    ...
    const ctx = await harness(adapter)
    const agent = (await ctx.agents.create({ sessionId: SessionId('unrenderable'), agentOptions: { provider: 'mock', model: 'mock' } })).agent
...
describe('driver bookkeeping edges', () => {
  it('rejects a direct turn invocation without a driver reservation', async () => {
    ...
    const agent = (await ctx.agents.create({ sessionId: SessionId('turn-without-reservation'), agentOptions: { provider: 'mock', model: 'mock' } })).agent
  ...
  it('closes an entered turn as blocked when its next step is rejected', async () => {
    ...
    const ctx = await harness(adapter)
    const agent = (await ctx.agents.create({ sessionId: SessionId('reject-next-step'), agentOptions: { provider: 'mock', model: 'mock' } })).agent
    let proposals = 0
  ...
  it('a request failure that concludes recovery after step/end closed keeps the boundary balanced', async () => {
    ...
    const ctx = await harness(adapter)
    const agent = (await ctx.agents.create({ sessionId: SessionId('finish-after-close'), agentOptions: { provider: 'mock', model: 'mock' } })).agent
    void LlmError
