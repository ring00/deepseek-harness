...
describe('agent/pre-step', () => {
  ...
  it('reports the request coordinates for initial and tool-continuation prompts', async () => {
    ...
    const agent = (await ctx.agents.create({ sessionId: SessionId('prompt-coordinates'), agentOptions: { provider: 'mock', model: 'mock' } })).agent
  ...
  it('publishes frozen input without replacing its identity', async () => {
    const adapter = new MockAdapter([textResponse('ok')])
    const ctx = await harness(adapter)
    const agent = (await ctx.agents.create({ sessionId: SessionId('owned-input'), agentOptions: { provider: 'mock', model: 'mock' } })).agent
  ...
  it('does not open another step when a completed turn rewrites pending input to empty', async () => {
    ...
    const ctx = await harness(adapter)
    const agent = (await ctx.agents.create({ sessionId: SessionId('empty-completed-continuation'), agentOptions: {
      provider: 'mock',
      model: 'mock',
    } })).agent
  ...
  it('stages inject and steer during pre-step for the entered turn', async () => {
    const adapter = new MockAdapter([textResponse('ok')])
    const ctx = await harness(adapter)
    const agent = (await ctx.agents.create({ sessionId: SessionId('pre-step-outbox'), agentOptions: { provider: 'mock', model: 'mock' } })).agent
    const entered = Promise.withResolvers<undefined>()
    const decision = Promise.withResolvers<PreStepDecision>()
  ...
  it('preserves input staged after the blocked batch was claimed', async () => {
    ...
    const ctx = await harness(adapter)
    const agent = (await ctx.agents.create({ sessionId: SessionId('blocked-pre-step-outbox'), agentOptions: { provider: 'mock', model: 'mock' } })).agent
    const entered = Promise.withResolvers<undefined>()
    const decision = Promise.withResolvers<PreStepDecision>()
  ...
  it('preserves later queued work when a step is rejected', async () => {
    ...
    const ctx = await harness(adapter)
    const agent = (await ctx.agents.create({ sessionId: SessionId('rejected-pre-step-order'), agentOptions: {
      provider: 'mock',
      model: 'mock',
    } })).agent
    ctx.on('agent/pre-step', async ({ messages }, next) => {
  ...
  it('preserves context-only injection staged after pre-step began', async () => {
    ...
    const ctx = await harness(adapter)
    const agent = (await ctx.agents.create({ sessionId: SessionId('rejected-pre-step-context'), agentOptions: { provider: 'mock', model: 'mock' } })).agent
    const entered = Promise.withResolvers<undefined>()
    const decision = Promise.withResolvers<PreStepDecision>()
    const disposeBlock = ctx.on('agent/pre-step', async () => {
      entered.resolve(undefined)
  ...
  it('leaves inbox state unchanged when its durable append fails', async () => {
    const adapter = new MockAdapter([])
    const ctx = await harness(adapter)
    const agent = (await ctx.agents.create({ sessionId: SessionId('rejected-pre-step-append-failure'), agentOptions: {
      provider: 'mock',
      model: 'mock',
    } })).agent
...
describe('worked example: a native hook plugin is just a cordis plugin on the seams', () => {
  ...
  it('the same plugin blocks a destructive prompt inside a no-step turn', async () => {
    ...
    const ctx = await harness(adapter)
    await ctx.plugin(NativeGuard)
    const agent = (await ctx.agents.create({ sessionId: SessionId('a2'), agentOptions: { provider: 'mock', model: 'mock' } })).agent
  ...
  it('HMR-safety: disposing the plugin fiber removes all four listeners', async () => {
    ...
    const agent = (await ctx.agents.create({ sessionId: SessionId('a3'), agentOptions: { provider: 'mock', model: 'mock' } })).agent
    send(agent, 'run rm -rf /')
    await waitForIdle(ctx, agent)
