...
- `ctx.agents.registerSetup(setup: AgentSetup): () => void` — contribute ordered setup after each caller callback and before publication. Removal aborts and drains active calls, prevents future calls, and invalidates a completed setup that has not yet published.
